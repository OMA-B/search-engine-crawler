...
def scrap_web(search_engine, URL, input_selector, keyword, search_result_title, next_selector):
    ...
    if 'duckduckgo' == search_engine:
        for num in range(10): # 10 will be replaced with max search number
            try:
                def click_next():
                    time.sleep(2)
...
for engine in search_engines:
    if engine == search_engine:
        requirements = search_engines[engine]
        scrap_web(search_engine=search_engine, URL=requirements[0], input_selector=requirements[1], keyword=keyword, search_result_title=requirements[2], next_selector=requirements[3])
